...
(function initOneTimeVideo() {
  ...
  console.log('Video init with fade started');

  // Listen for the video to end OR use a timeout (in case video doesn't load or ends immediately)
  function triggerFade() {
    console.log('Starting fade animations');
    
    // Video fade out
    video.classList.add('fade-out');
    
    // Hero content fade in AFTER 1.25 EXTRA SECONDS
    setTimeout(() => {
    ...
    setTimeout(() => {
      console.log('Removing video from DOM');
      if (video.parentNode) {
        video.remove();
      }
    }, 2500);
  ...
  // Try to listen to video ended event first
  video.addEventListener('ended', triggerFade, { once: true });
  
  // Fallback: if video doesn't end (e.g., no video, or looped), use a timeout
  setTimeout(triggerFade, 3000);

  // IntersectionObserver to remove video if user scrolls away
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (!entry.isIntersecting) {
        console.log('Hero section not visible, removing video');
        if (video.parentNode) {
          video.remove();
        }
